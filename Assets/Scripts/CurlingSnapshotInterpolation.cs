...
namespace Curling
{
    public struct CurlingSnapshot: Snapshot
    {
        ...
        public CurlingSnapshot(double remoteTimestamp, double localTimestamp, Vector3 position, Quaternion rotation)
        {
        ...
        public static CurlingSnapshot Interpolate(CurlingSnapshot from, CurlingSnapshot to, double t)
        {
            // NOTE:
            // Vector3 & Quaternion components are float anyway, so we can
            // keep using the functions with 't' as float instead of double.
            return new CurlingSnapshot(
